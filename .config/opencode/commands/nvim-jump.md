...
- If line or col are unknown, use `1`.

## Populating quickfix

1. Write a quickfix file with one entry per line in this format:

```
/absolute/path/to/file:line:col: message
```

2. Then load it into Neovim:
...
- Set `$NVIM_SERVER` to the Neovim socket path if it differs from `/tmp/nvimsocket`.
- Requires `nvr` (neovim-remote) in PATH.
- Neovim must be running with `--listen` on the matching socket.
